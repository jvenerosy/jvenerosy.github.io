...
C'est l'état d'un élément, il peut être de type :
 ...
 - **block** : c'est avec le inline l'était le plus répandu. Les `<div>`, `<p>` ou encore `<hx>` sont de type "block", ces élément se succèdent les uns en dessous des autres, prennent de base toute la largeur de leur conteneur (ou parent).
