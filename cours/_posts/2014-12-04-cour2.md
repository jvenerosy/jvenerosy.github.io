...
![Validateur OK](validator-ok.PNG)
{: .image}
...
Je vous invite à consulter le très bon article [d'OpenClassRooms sur la sémantique](http://openclassrooms.com/courses/la-semantique-1), il nous montre les principales balises leur champs d'utilisation.
</div>
...
Petite parenthèse sur les caractères spéciaux. Comme nous l'avons vous, nous utilisons beaucoup plus de caractères que nos amis anglo-saxons. Qui n'avaient pas prévu qu'un beau jour, nous aussi pourrions utiliser, et programmer l'internet :)  
